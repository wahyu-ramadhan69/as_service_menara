...
export async function PUT(
  ...
  try {
    ...
    if (status.data.data.status === "stopped") {
      return respondWithError("Hidupkan vm untuk melakukan restart", 200);
    }

    const shutdown = await axios.post(
      `${process.env.PROXMOX_API_URL}/nodes/${node}/qemu/${id}/status/reset`,
      {},
      { headers, httpsAgent }
    );
    ...
    while (!taskFinished) {
      ...
      if (statusResponse.data.data.status === "running") {
        taskFinished = true;
      } else {
        await new Promise((resolve) => setTimeout(resolve, 3000));
      }
    ...
    return respondWithSuccess(
      `VM with ID ${id} has been PowerOn successfully`,
      200
    );
  } catch (error) {
